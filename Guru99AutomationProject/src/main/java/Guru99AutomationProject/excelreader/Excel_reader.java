package Guru99AutomationProject.excelreader;
...
public class Excel_reader {
	...
	public Excel_reader(String path2) {
		this.path = path;
		try {
	...
	/*public String Excel_Reader(String path) {
		this.path = path;
		try {
			fis = new FileInputStream(path);
			workbook = new XSSFWorkbook(fis);
	...
	public String[][] getDataFromSheet(String sheetName, String ExcelName) {
		String dataSets[][] = null;
			try {
				...
				// count number of active rows ie, rows containing some text(heading + login data)
				int totalRow = sheet.getLastRowNum() + 1;
				
				// count number of active columns in row first row containing heading.
				int totalColumn = sheet.getRow(0).getLastCellNum();
				
				// Create array of rows and column. (totalRow - 1 : it will exclude heading row from excel.)
				//total row = 6(to print) (index from 0 to 5) if total rows-1 then matrix becomes [5,2] 
